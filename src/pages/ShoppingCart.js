...
export default function ShoppingCart(props) {
  ...
  const getCart = async () => {
    let tempCart = await Axios.post(
      "https://api-noobegg.up.railway.app/api/getCart",
      { userID: user.UserID }
    );
    ...
    if (tempCart.data.message != null) {
      ...
      for (let i = 0; i < tempCart.data.length; i++) {
        subt += tempCart.data[i].ProductPrice * tempCart.data[i].quantity;
        //console.log(i)
        if (i < tempCart.data.length - 1) {
          pStr +=
            tempCart.data[i].ProductID + ":" + tempCart.data[i].quantity + ",";
          //console.log(pStr)
          setpS(pStr);
        } else {
          pStr += tempCart.data[i].ProductID + ":" + tempCart.data[i].quantity;
          // console.log("In else")
          // console.log(pStr)
          setpS(pStr);
        }
  ...
  const getUserOrders = async (uID) => {
    let orders = await Axios.get(
      `https://api-noobegg.up.railway.app/api/getUserOrders/${uID}`
    );
  ...
  const getCodes = async () => {
    let tempCodes = await Axios.get(
      "https://api-noobegg.up.railway.app/api/getCodes"
    );
  ...
  const deleteItem = async (itemID) => {
    ...
    let message = await Axios.delete(
      `https://api-noobegg.up.railway.app/api/deleteItem/${itemID}/${user.UserID}`
    );
  ...
  const checkout = async () => {
    ...
    for (let i = 0; i < itemList.length; i++) {
      ...
      Axios.post("https://api-noobegg.up.railway.app/api/getProduct", {
        ...
        if (itemList[i].quantity > response.data[0].ProductStock) {
          setStockErrMsg(
            "Sorry, we dont have that many " +
              response.data[0].ProductName +
              "(s) available. Please check available stock and change your order accordingly."
          );
          return;
        }
      });
    ...
    for (let i = 0; i < itemList.length; i++) {
      Axios.put("https://api-noobegg.up.railway.app/api/updateProductStock", {
        productID: itemList[i].ProductID,
        newStock: itemList[i].quantity,
      });
    }

    //Clear cart WORKS
    Axios.delete(
      `https://api-noobegg.up.railway.app/api/clearCart/${user.UserID}`
    );
      ...
      <ToastContainer className="p-3" position={"bottom-start"}>
        ...
        >
          ...
          <Toast.Body className="text-white">
            The code typed in is not valid.
          </Toast.Body>
        </Toast>
      </ToastContainer>
      <Row>
                  ...
                  return (
                    <Card key={product.productID}>
                      <Card.Body>
                        ...
                        <Col>Amount: {product.quantity}</Col>
                        <Col>
                          <Button
                            variant="danger"
                            onClick={() => {
                              clickedDelete(product.ProductID);
                            }}
                          >
                            Delete Item
                          </Button>
                ...
                <Col>
                  <Card>
                    <Card.Body>
                      ...
                      {validCode ? (
                        <Col>
                          <Badge bg="success">Code {userCode} applied.</Badge>
                        </Col>
                      ) : (
                        <div>
                          <Col>
