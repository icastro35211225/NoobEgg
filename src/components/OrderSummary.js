...
export default function OrderSummary(props) {
  ...
  const getOrderUser = async (uID) => {
    let oUser = await Axios.get(
      `https://api-noobegg.up.railway.app/api/getUser/${uID}`
    );
  ...
  const getOrder = async () => {
    let ord = await Axios.get(
      `https://api-noobegg.up.railway.app/api/getOrder/${orderID}`
    );
  ...
  const getProducts = async () => {
    ...
    for (let i = 0; i < prodArr.length; i++) {
      ...
      let currProdObj = await Axios.post(
